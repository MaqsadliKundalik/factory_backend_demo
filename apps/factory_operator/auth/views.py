...
from apps.factory_operator.serializers.auth import LoginSerializer, ProfileSerializer, LogoutSerializer
...
class LoginAPIView(APIView):
    ...
    @swagger_auto_schema(
        ...
        operation_summary="Factory Operator Login",
    ...
    def post(self, request: HttpRequest | Request):
        ...
        operator = FactoryOperator.objects.filter(phone_number=data["phone_number"]).first()
        if operator is None:
            raise NotFound("Boshqaruvchi topilmadi.")

        if not operator.check_password(data["password"]):
...
class ProfileAPIView(APIView):
    ...
    @swagger_auto_schema(
        tags=["Factory Operator Auth"],
        operation_summary="Factory Operator Profile",
...
class LogoutAPIView(APIView):
    ...
    @swagger_auto_schema(
        tags=["Factory Operator Auth"],
        operation_summary="Factory Operator Logout",
