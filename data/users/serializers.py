...
class FactoryUserSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        representation["whouses"] = [
            {"id": whouse.id, "name": whouse.name} for whouse in instance.whouses.all()
        ]
    ...
    def create(self, validated_data):
        ...
        if Driver.objects.filter(phone_number=phone_number).exists():
            raise serializers.ValidationError(
                {
                    "phone_number": "Bu telefon raqami allaqachon haydovchi tomonidan ro'yxatdan o'tgan"
                }
            )
    ...
    def update(self, instance, validated_data):
        ...
        if "phone_number" in validated_data:
            ...
            if (
                FactoryUser.objects.filter(phone_number=new_phone_number)
                .exclude(pk=instance.pk)
                .exists()
            ):
                raise serializers.ValidationError(
                    {
                        "phone_number": "Bu telefon raqami allaqachon boshqa foydalanuvchi tomonidan ro'yxatdan o'tgan"
                    }
                )
