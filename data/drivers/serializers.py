...
class DriverSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        representation["photo"] = (
            FileSerializer(instance.photo).data if instance.photo else None
        )
    ...
    def create(self, validated_data):
        ...
        if Driver.objects.filter(phone_number=phone_number).exists():
            raise serializers.ValidationError(
                {"phone_number": "Bu telefon raqami allaqachon ro'yxatdan o'tgan"}
            )
        ...
        driver = Driver.objects.create(
            PRODUCTS_PAGE=True, ORDERS_PAGE=True, TRANSPORTS_PAGE=True, **validated_data
        )
        return driver
    ...
    def update(self, instance, validated_data):
        ...
        if "phone_number" in validated_data:
            ...
            if (
                Driver.objects.filter(phone_number=new_phone_number)
                .exclude(pk=instance.pk)
                .exists()
            ):
                raise serializers.ValidationError(
                    {
                        "phone_number": "Bu telefon raqami allaqachon boshqa haydovchi tomonidan ro'yxatdan o'tgan"
                    }
                )
            ...
            if FactoryUser.objects.filter(phone_number=new_phone_number).exists():
                raise serializers.ValidationError(
                    {
                        "phone_number": "Bu telefon raqami allaqachon foydalanuvchi tomonidan ro'yxatdan o'tgan"
                    }
                )
...
class DriverPasswordChangeSerializer(serializers.Serializer):
    new_password = serializers.CharField(
        write_only=True, required=True, validators=[password_validator(8)]
    )
