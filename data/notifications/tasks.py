...
def _get_sessions(to_role, to_user_id=None):
    if to_role == 'driver':
        ...
        qs = FactoryUserSession.objects.filter(factory_user__role=to_role, fcm_invalid=False)
        if to_user_id:
...
def _send_batch(sessions, notification):
    ...
    for i, resp in enumerate(batch_response.responses):
        if not resp.success:
            ...
            if 'registration-token-not-registered' in text or 'UNREGISTERED' in text.upper():
...
def send_notification(notification_id):
    ...
    for i in range(0, total, BATCH_SIZE):
        batch_sent, batch_failed = _send_batch(sessions[i:i + BATCH_SIZE], notification)
        sent += batch_sent
        failed += batch_failed
